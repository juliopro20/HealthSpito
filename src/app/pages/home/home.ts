...
import { Component, ElementRef, OnInit, OnDestroy, ViewChild, Renderer2, inject, DOCUMENT, HostListener } from '@angular/core';
...
export class Home implements OnInit, OnDestroy {
  ...
  // Original cards (fixed icon spacing issue)
  originalCards: Card[] = [
    {
      icon: 'how_to_reg',
  ...
  currentIndex = this.originalCards.length;
  gap = 20;
  isAnimating = false;
  ...
  // Automatically recalculate position if window size or orientation changes
  @HostListener('window:resize')
  onResize(): void {
    this.updateOffset();
  }

  getActualCardWidth(): number {
    const firstCard = this.elRef.nativeElement.querySelector('.carousel-card');
    return firstCard ? firstCard.getBoundingClientRect().width : 300;
  }
  ...
  updateOffset(): void {
    const cardWidth = this.getActualCardWidth();
  ...
  getCardScale(index: number): number {
    return index === this.currentIndex ? 1.03 : 1.0;
  }
